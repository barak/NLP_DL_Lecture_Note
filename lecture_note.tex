...
\usepackage[pdftex,hidelinks,pagebackref]{hyperref}
...
\abstract{
    ...
    note, I found this fascinating note by \citet{goldberg2015primer}
    on neural network models for natural language processing. This note
...
Schuetze, ``\textit{even practically-minded people have to confront the issue of
what prior knowledge to try to build into their model}''
\citep{manning1999foundations}.

\section{Route we will \textit{not} take}
...
constraints on a set of \textit{generating rules}, or production rules. 
...
\textit{generates} a natural language. But, then, the obvious question to follow is
...
these to linguists and continue on to our journey of \textit{building a machine
...
that there's a universal grammar \textit{built in} our brain. In other words, let's
...
In our case, we are more interested in a \textit{machine} hearing that sound, or a
sentence from here on. When a machine heard this sentence, what would/should a
\textit{language understanding machine} do to understand a language, or more simply
...
Let's take an example sentence ``\textit{Our company is training workers}''
...
\begin{figure}[ht]
    ...
    \caption{A parse of ``\textit{Our company is training workers}''}
    \label{fig:manning_parse}
\end{figure}

The machine hears the sentence ``\textit{Our company is training workers}'' and
...
For instance, given a question ``\textit{Who is training workers?}'', the machine
can answer by noticing that the question is asking for the subject of the verb
phrase
``\textit{is training}'' acted on the object ``\textit{workers}'' and that the subject
is ``\textit{Our company}''.
...
In this case, we assume to know \textit{the} set of rules in advance and let the
...
generating rules from a given sentence is \textit{understanding}, and answering a
question based on this understanding, \textit{using}, is a separate activity.
...
require its use, or downstream tasks. In this road that we will \emph{not} take
...
\section{Road we \textit{will} take}
\label{sec:intro}

\subsection{Language as a Function}

In this course, we will view a natural/human language as ``\textit{a system
...
system or behaviour of \textit{communication} as a language. Furthermore, this view
...
one's will or intention.\footnote{
    Chomsky does not agree: ``\textit{it is wrong to think of human use of language
...
the behaviourism of B. F. Skinner (``\textit{necessary mediation of reinforcement by
...
language. \textit{Language is a function of communication}. 
...
approximation is \emph{the} core of machine learning. Classification is a
...
Hence, we \emph{weight} the distance between the approximated $\hat{\vy}$ and the
...
cost function $C(\TT)$. This is equivalent to computing the \emph{expected}
...
is referred to as \emph{expected risk minimization} \citep{Vapnik1995}.
...
minimizes the \emph{expected} cost.


\section{Learning as Optimization}

We often call this process of finding a good set of parameters that minimizes
the expected cost \emph{learning}. This term is used from the perspective of a
machine which implements the function $f_{\TT}$, as it \emph{learns} to
...
mentioning that learning is \emph{optimization}. We have a clearly defined
...
By an \emph{iterative} optimization, I mean an algorithm which refines its
...
worthwhile to note that most iterative optimization algorithms are \emph{local},
...
convergence point.\footnote{
    There are \emph{global} optimization algorithms, but they are out of scope
...
algorithm depends entirely on the gradient of the cost function.\footnote{
    From here on, I will use the cost function to refer to the \emph{empirical}
    cost function.
} 
...
the \emph{opposite} direction toward which we want to move the parameters. See
...
This enormous computational cost involved in each GD step has
motivated the \emph{stochastic gradient descent} (SGD) algorithm
\citep{Robbins1951,Bottou1998}.

First, recall from Eq.~\eqref{eq:empirical_cost} that the cost function we
minimize is the \emph{empirical} cost function $\tilde{C}$ which is the
sample-based approximation to the \emph{expected} cost function $C$. This
...
subset of the training set. We call this small subset a \emph{minibatch}.
...
iteratively refining its set of parameters, in most cases using \emph{stochastic
...
function which is the one we want to minimize in order to work well
with \emph{any} pair of input $\vx$ and output $\vy$. Instead, we have
access to the
...
most important trick is \emph{early stopping}, which is only applicable when
...
effectiveness, this early stopping strategy has become \emph{de facto} standard
...
searching for the best \emph{hypothesis} over the entire space $\HH$ of
...
$M \in \HH$ to denote one hypothesis.\footnote{
    $M$, because each hypothesis corresponds to one learning \emph{machine}.
...
Given a definition of expected cost function, we can \emph{score} each hypothesis
...
issue of \emph{overfitting} which means that the optimization algorithm overshot
...
This is one way to do \emph{model selection}, and we will talk about another way
to do this later.

\section{Evaluation}

But, wait, if this is an argument for using the validation cost to \emph{early
...
\emph{three} partitions. We call them a training set $D_{\train}$, a validation
...
One most important lesson here is that you \emph{must never look at a test set}.
...
Second, because we were \emph{given} $\vx$ (meaning we did not have much control
...
Let us call this quantity $\phi(x)$, or equivalent \emph{feature}, such that
\begin{align}
    \label{eq:feat_month}
...
called \emph{feature extraction}. This feature extraction is an important step in
...
anything including a \emph{distribution} \citep{Bridle1990,denker1991transforming,bishop1994mixture}.
...
function now returns \emph{the parameters of the distribution} $\TT(\vx)$ given
the input $\vx$. 
...
\end{align*}
which is often called a \emph{cross entropy cost}. In the case of
...
instead of returning \emph{the} answer out of these two possible answers.}
...
$K$ (unbounded) real values followed by a so called \emph{softmax}
...
nothing but a \emph{function}. A function of a function is a function, right? In
...
vector $\phi_0(\vx)$ is \emph{much} larger than that of $\vx$, the function
...
themselves less than a decade ago (as of 2015) who call this model an
\emph{extreme learning machine} \citep{Huang2006}. There have been some debates about
...
feature extraction \emph{layers} in Eq.~\eqref{eq:layer} between the input and
...
Both of these derivatives \emph{share} $\frac{\partial C_x}{\partial
...
computational graph (multilayer perceptron) are \emph{shared}. At the end of the
...
\begin{align*}
    ...
    x^\top.
\end{align*}
Note that all the vectors in this lecture note are \emph{column} vectors.
...
This procedure, presented as two examples, is called a \emph{backpropagation}
...
as \emph{local error} $\delta$ and so on. But, personally I find it much easier
...
$\frac{\partial C_y}{\partial \sigma^1}$ passed \emph{back} from the function
...
backpropagating the derivatives is that each function node implements both \emph{forward computation} as well as \emph{backward computation}. In the backward
...
a \emph{computational graph of gradient}, where the output is the derivatives of
...
\subsection{What if a Function is \emph{not} Differentiable?}
...
%become \emph{the} standard in solving many challenging computer vision and speech
%tasks. 
...
What do I mean by a \emph{variable length input}? A variable length input $x$ is
a \emph{sequence} where each input $x$ has a different number of elements. For
...
a single function {\sc add1} is applied to each symbol in the sequence \emph{one
...
Both {\sc add1} and CPU are \emph{hard coded} in the sense that they do what they
...
(linguistic) symbols and use a memory in order to \emph{understand natural
languages}.

To build this parametric recursive function\footnote{
    In neural network research, we call this function a \emph{recurrent neural
    network.}
}
...
vector. It often helps to \emph{time index} the memory vector as well, such that
...
think of how we use this recursive function to \emph{return} an output $y$. As
...
time.) Also, each transformation layer \emph{shares} the parameters $\mW$ and
$\mU$.\footnote{
    ...
    \left[ \begin{array}{c} \vx \\1 \end{array} \right]
        = \mW \vx + \vb
    \end{align*}
    Note that as I have declared before all vectors are \emph{column} vectors.
...
\begin{align*}
    ...
    \frac{\partial f_i}{\partial x}.
\end{align*}
This means that when multiple derivatives are \emph{backpropagated} into a single
node, the node should first \emph{sum them} and multiply its summed derivative
...
There is a task in natural language processing called \emph{sentiment analysis}.
...
assume for now that the input is a \emph{sequence of words}, where each word
...
affine-transform $\vh_l$ followed by the \emph{softmax} function to obtain the
...
each word in a sentence as one of \emph{noun}, \emph{verb}, \emph{adjective} and
\emph{others}. As an example, given the following input sentence $x$
\begin{align*}
...
recurrent neural network, we will use the \emph{immediate memory state} to
...
From this output distribution at time step $t$, we can define a \emph{per-step},
\emph{per-sample} cost function:
\begin{align}
...
In other words, this means that the recurrent neural network is \emph{not} taking
...
\emph{inference}\footnote{
    Okay, I confess. The term \emph{inference} refers to a much larger class of
...
probabilities later on. It is highly related to the so-called \emph{garden path
...
the output sequence at each time step. This procedure is called \emph{beam
search}, and we will discuss more about this in a later lecture on neural
machine translation.

\section{Gated Recurrent Units}

\subsection{Making Simple Recurrent Neural Networks \emph{Realistic}}
...
that it \emph{refreshes} the whole memory state at each time step. This is almost
...
With this binary vector, which I will call an \emph{update gate}, a new memory
...
vector to represent. Let me call it a \emph{reset gate} $\vr \in \left\{ 0,
1\right\}^{n_h}$:
\begin{align*}
    r_i = \left\{ 
        \begin{array}{l l}
...
This reset gate can be multiplied to the register values \emph{before} being used
by the instruction at time $t$.\footnote{
    It is important to note that this is \emph{not} resetting the actual values
...
neural network more realistic into one piece. At each time step, the \emph{candidate} memory state is computed based on a subset of the elements of the
...
Now here goes a big question: \emph{How are the update $\vu$ and reset $\vr$
gates computed?}

If we stick to our analogy to the CPU, those gates must be pre-configured \emph{per instruction}. Those binary gates are dependent on the instruction. Again
...
computing a recurrent \emph{function} based on the input symbol and the memory
...
cost function efficiently \emph{and} exactly with these discrete activation
...
\end{align*}
This approach makes these gates \emph{leaky} in the sense that they always allow
...
now decides \emph{how much} information from the previous memory state will be
...
how much information will be \emph{kept} from the previous memory state.
...
parameters.\footnote{
    Note that this is not \emph{the} formulation available for computing the
...
\emph{gated recurrent unit} (GRU), and a recurrent neural network having this GRU
...
and the output $\vh_t$. The output is a small subset of the \emph{hidden} memory
state, and only this subset of the memory state is visibly \emph{exposed} to any
...
so-called \emph{output} gate $\vo$ to achieve this goal. Similarly to the reset
...
decides how much information from the memory state will be \emph{forgotten},
while the input gate controls how much information about the new input
(consisting of the input symbol and the previous output) will be \emph{inputted}
...
Let us go back to the \emph{simple recurrent neural network} which uses the
...
than $1$, the norm of $\vh_l$ will \emph{explode}, i.e., $\|\vh_l\| \to \infty$.
...
This happens, because the nonlinearity $g$ is \emph{unbounded}. If $g$ is bounded
...
nonlinear function a \emph{simple recurrent neural network}. 
...
are \emph{the} nonlinear functions that one should use. This seems quite
convincing for recurrent neural networks, and perhaps so for feedforward neural
networks as well, if the network is \emph{deep} enough.
...
Both of these functions' derivatives are \emph{bounded}:
\begin{align}
...
will \emph{vanish}, i.e., $\|\frac{\partial \vh_{l_1}}{\partial \vh_{l_0+1}}\|
\to 0$, exponentially fast.

What if the recurrent activation function $f$ is \emph{not} linear at all? Let's
...
$\frac{1}{\lambda}$, it is \emph{likely} that this temporal derivative of
$\vh_{l_1}$ with respect to $\vh_{l_0+1}$ will \emph{explode}, meaning that its
...
$0$. The former case is referred to as \emph{exploding gradient}, and the latter
\emph{vanishing gradient}. These cases were studied already at the very early
...
algorithm is referred to as \emph{gradient clipping}
...
directly reflects the degree to which the $t$-th output $y_t$ \emph{depends} on
the $t'$-th input $x_{t'}$, \emph{according to the network}.  To put it in
another way, $\frac{\partial \log p(y_t=y_t^*|\ldots)}{\partial\phi(x_{t'})}$
reflects how much dependency the \emph{recurrent neural network has captured the
...
have already seen that this term can easily \emph{vanish} toward zero with a high
...
effectively forcing the gradient \emph{not} to vanish.
...
What this implies is that the GRU effectively introduces a \emph{shortcut} from
...
dependency across many time steps, but this inevitably carries \emph{unnecessary}
information as well. This means that much of the representational power of the
output function $g_{\text{out}}(\vh_t)$ is wasted in \emph{ignoring} those
...
gates are \emph{not} fixed but are adaptive with respect to the input. If the
...
define the \emph{likeliness} of a sentence, because there are many different
types of unlikeliness. For instance, a sentence ``Colorless green ideas sleep
furiously'' from \citet{chomsky2002syntactic} is \emph{unlikely}
...
response, and this function is \emph{not} given but learned via observing others'
...
often. In other words, statistical approaches to \emph{language modeling} may
...
including myself, quite uneasy about this type of \emph{model-based} approaches. 
...
has bugged me a lot: \emph{what is the minimal linguistic unit?} 
...
level at which we want to model the process of \emph{understanding} natural
...
However, this raises a follow-up question on the definition of a word: \emph{What
is a word?}
...
\begin{align*}
    S = (w_1, w_2, \ldots, w_T).
\end{align*}
Each symbol is one element from a \emph{vocabulary} $V$ which contains all
possible symbols:
\begin{align*}
...
This is equivalent to counting how many times $S$ occurs in the data.\footnote{
    A data set consisting of (written) text is often referred to as a \emph{    corpus}. 
}
...
do not occur in the data. As soon as a given sentence does not appear \emph{exactly} as it is in the corpus, this estimator will say that there is a \emph{zero} probability of the given sentence. Although the sentence ``I like llama''
is a likely sentence, according to this estimator in
Eq.~\eqref{eq:sentence_mle}, it will be deemed extremely unlikely.

This problem is due to the issue of \emph{data sparsity}. Data sparsity here
...
sentences grows hints us a straightforward approach to addressing this: \emph{limit the maximum length of phrases/sentences we estimate a probability on}.
...
sentence is \emph{predictable} based on the $n-1$ preceding symbols. This is in
...
Since it is a
\emph{conditional} probability, we need to rewrite it according to the definition
...
\emph{Note that I am missing many references this section, as I am writing this
...
``\textit{data sparsity is almost always an issue in statistical modeling}''
...
One way to alleviate this problem is to assign a small probability to all \emph{unseen} $n$-grams. At least, in this case, we will assign some small, non-zero
...
%\emph{back-off} to lower-order $n$-grams when a given
...
still not totally satisfactory, because of the lack of \emph{generalization}.
What do I mean by \emph{generalization} here?
...
Consider an example where three trigrams\footnote{
    ...
    \citet{manning1999foundations}: ``\textit{
        these
...
In other words, there \emph{exist} relevant $n$-grams in the training corpus, but
...
model notice that ``llama'' is a ``\emph{a domesticated pack animal of the camel
...
equivalent to \emph{slicing out a single row from the matrix}.
...
representation is often called a \textit{word embedding} and should reflect the
...
vector space and often referred to as a \emph{context vector}.
...
\end{align*}
We will call the resulting vector $\vh$ a \emph{context vector}.
...
The ratio between $p_j^l$ and $p_k^l$ is then\footnote{
    Note that both $p_j^l$ and $p_k^l$ are positive due to our use of \emph{softmax}.
}
\begin{align*}
...
It is important to notice that neural language model \emph{must} project
...
vector, the neural language model will \emph{have to} assign a high probability
...
different target words (via target word vectors $\vu_k$), by exploiting \emph{co-occurrences} of words. In this example, the neural language model learned
...
This is about time someone asks a question why we are only considering the \emph{preceding} words when doing language modelling. Is it a good assumption that the
...
\emph{Markov random field (MRF) language model} \citep{jernite2015fast}.
...
surrounding words with \emph{undirected} edges, and these edges represent the
...
must first notice that the conditional probability of $w^i$ \emph{only} depends
on the values of other words included in its \emph{Markov blanket.} In the case
of Markov random fields, the Markov blanket of a random variable is defined as a
set of all \emph{immediate neighbours}, and it implies that the conditional
...
This conditional probability is a shallow neural network with a single \emph{linear} hidden layer whose input are the context words ($n$ preceding and $n$
...
a skip-gram model \emph{implicitly} factorizes a PPMI matrix. 
...
they are purely \emph{unsupervised}, meaning that all we need is a large corpus
...
corpus of \emph{unlabelled} text, we get good vectors for those linguistic units
...
issue which is that we have only \emph{two} training examples!  All the word
...
assumption that \emph{similar} input will have similar sentiments. This
...
using unlabelled training examples, we can train a good classifier\footnote{
    ...
    classifies \emph{unseen} test examples well. See
    Sec.~\ref{sec:model_selection}.
} 

And, guess what? We have access to this lower-dimensional manifold, which is
represented by the set of \emph{pretrained} word vectors $\mE$. Believing that
...
sentiment classification of words. In fact, this aspect of similarity will \emph{hurt} the sentiment classifier, because a positive word ``happy'' will be close
...
into the very \emph{recurrent neural network language model} in this section.
...
$\vh^t$ by simply reading the input sentence \emph{once}. In other words, we can
let the recurrent neural network read one word $w^t$ at a time, update the
memory state $\vh^t$ and compute the conditional probability of the \emph{next}
...
This language model is called a \emph{recurrent neural network language
...
The most commonly used metric is a \emph{perplexity}. In the context of language
...
assigning to a \emph{correct next word} given all the previous words. Again,
...
\emph{on average}.  Therefore, \emph{a better language model is the one with a lower
...
Finally, we have come to the point in this course where we discuss an \emph{actual} natural language task. In this chapter, we will discuss how translation
...
\end{align}
Looking at it in this way, it is clear that this is nothing but \emph{conditional
...
minimizing the \emph{negative} log-likelihood (see
...
\end{align}
consisting of $N$ training \emph{pairs}.
...
Eqs.~\eqref{eq:translation_f}--\eqref{eq:condlm}, it is a \emph{sentence-to-sentence} translation task. We approach this problem by building a
...
is often called a \emph{parallel corpus}. It is not easy to think of documents
...
proceedings are digitally available and called \emph{Hansards}. You can check it
...
Prayers of the 2nd Session, 41st Parliament, Issue 152:\footnote{
    This is one political lesson here: \emph{Canada is still headed by the Queen of
    the United Kingdom}.
}
\begin{itemize}
    \itemsep 0em
...
big question follows: \emph{how do we evaluate this model}?
...
the machine translation system that is \emph{not} a phrase-based statistical
...
We call this recurrent neural network an \emph{encoder}, as it encodes the source
...
denote this vector as $\vc$ and call it a \emph{context vector}.

We now need to design a \emph{decoder}, again, using a recurrent neural network.
...
\emph{trajectory} in a continuous vector space, and each point in the trajectory
...
sentences. Hence, we must resort to some kind of \emph{approximate search}. Wait,
\emph{search}? Yes, this whole procedure of finding the most likely translation
...
There is a big problem of this \emph{greedy search}. That is, as soon as it makes
...
the greedy search is \emph{too} greedy. The question is then whether there is
...
It should be clear that this procedure, called \emph{beam search} and shown in
...
\end{align*}
In the case of $K=1$, i.e., \emph{greedy search}, we choose $a$. If $K=2$, we
will keep $(a)$ and $(c)$. 
...
beam search becomes \emph{complete}, refer to, e.g.,
...
space of all possible source sentences is \emph{countable}, while the context
vector space $\left[ -1, 1 \right]^d$ is \emph{uncountable}. There exists a
...
there was that \citet{sutskever2014sequence} used a \emph{much}
...
sentence \emph{should} be fully represented as a single vector. For instance,
...
single vector. The question we are more interested in is whether \emph{there
...
with a \emph{set $C$ of context vectors} $\vh_t$'s. This is achieved by having
...
which we will refer as a \emph{forward recurrent neural network}, reads the
source sentence as usual and results in a set of \emph{forward memory states}
$\overrightarrow{\vh}_t$, for $t=1,\ldots,T_x$. The second recurrent neural
network, a \emph{reverse recurrent neural network}, reads the source sentence
in a reverse order, starting from $x_{T_x}$ to $x_1$. This reverse network will
output a sequence of \emph{reverse memory states} $\overleftarrow{\vh}_t$, for
$t=1, \ldots, T_x$.
...
$\overleftarrow{\vh}_t$ to form a \emph{context-dependent vector} $\vh_t$:
\begin{align}
    \label{eq:cd_rep}
    ...
    \end{array} \right]
\end{align}
We will form a \emph{context set} with these context-dependent vectors $c=\left\{
\vh_1, \vh_2, \ldots, \vh_{T_x}\right\}$. See Fig.~\ref{fig:bienc} for the
graphical illustration of this process.

Now, why is $\vh_t$ a \emph{context-dependent vector}? We should look at what the
...
\tilde{y}_2, \ldots, \tilde{y}_{t-1})$ and the context vector\footnote{
    We will shortly switch to using a context \emph{set} instead.
}
...
informative of which target word should be generated at time $t$ \emph{after}
generating the first $t-1$ target words \emph{given} the source sentence. In
order to do so, $\vz_t$ must encode \emph{what have been translated so far} among
the words that are supposed to be translated (which is encoded in the context
vector $\vc$.) Let's keep this in mind.

In order to compute the new memory state $\vz_t$ with a \emph{context set}
...
how \emph{relevant} it is for translating a next target word. This scoring needs
...
\end{align}
Conceptually, the score $e_{j,t}$ will be computed by \emph{comparing}
...
\end{align}
We call these normalized scores the \emph{attention weights}, as they correspond
to how much the \emph{decoder attends} to each of the context vectors. This whole
process of computing the attention weights is often referred to as an \emph{attention mechanism} \citep[see, e.g.,][]{cho2015describing}.
...
an encoder and a decoder with the attention mechanism, an \emph{attention-based
...
The question is how we can avoid these degenerate cases. Or, \emph{is it
...
differ from others' answer: \emph{no}.
...
This incorporation of \emph{prior knowledge} however should not put a \emph{hard
...
computes an \emph{alignment} matrix $A \in \left[ 0, 1\right]^{|X| \times |Y|}$:
\begin{align*}
    A = \left[ 
        \begin{array}{c c c c}
...
(based on its \emph{context-dependent vector representation} from
...
decide to consider each sentence as a sequence of \emph{characters},\footnote{
...
machine translation uses a \emph{bidirectional} recurrent neural network in order
to obtain a context set. Each vector in the context set was considered a \emph{context-dependent vector}, as it represents what the center word means with
...
Furthermore, it is possible to \emph{pretrain} this feature extractor by training
the whole multilayer perceptron on a separate classification dataset.\footnote{
    ...
    become a \emph{de facto} standard in many of the computer vision tasks
...
the recurrent neural network decoder for the task of \emph{image caption
generation}. 
...
location with respect to the whole image, meaning each context vector is a \emph{spatially-localized, context-dependent image descriptor}. This was achieved by
using the last \emph{convolutional} layer's activations of the pretrained deep
convolutional network instead of the last \emph{fully-connected} layer's. See
...
speech, a listener's mental state and \emph{the surrounding world} to the
...
original goal is to build a \emph{natural language function}, all we need to do
...
information on ``Facebook'' the social network site. Having all the \emph{current} world knowledge encoded in the model's parameters is not ideal in this
sense. 
...
model a \emph{sentence} probability $p(S)$. This is a bit weird in the sense that
...
to emphasize is the importance of \emph{not looking at a held-out test set.} One
...
In Chapter~\ref{chap:nn}, we finally talked about \emph{deep} neural networks, or
